...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    dscores = margins / num_train
    h1 = X.dot(W1) + b1#前一步的输出(这一步的输入)
    h1[h1 < 0] = 0 #ReLU
    ...
    dh1 = np.dot(margins, W2.T) #这句不懂为什么dh1是这两个的乘积
    # dh1 = dh1 / num_train #仅供测试
    dh1[h1 <= 0] = 0 #没有激活的部分对Loss没有贡献,因此传回来的梯度也应设为0    
    dW1 = np.dot(X.T, dh1)
    dW1 /= num_train
    dW1 += 2 * reg * W1
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in range(num_iters):
      ...
      #########################################################################
      indices = np.random.choice(num_train, batch_size) #出现了重复.抽样可以允许同一个样本被选中多次吗?
      X_batch = X[indices, :]
      y_batch = y[indices]
      #########################################################################
      #                             END OF YOUR CODE                          #
